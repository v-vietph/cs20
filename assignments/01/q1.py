...
out = tf.case([(tf.greater(x, y), lambda: x - y)], default = lambda: 0.0 )
print(sess.run(out))
# YOUR CODE
...
express = tf.reduce_mean(x-y) 

def f1(): return tf.square(express)
def f2(): return tf.abs(x-y)
z = tf.cond(express>0, f1, f2)
