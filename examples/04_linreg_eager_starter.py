...
DATA_FILE = '/home/hp/work_space/optimization/CS20/stanford-tensorflow-tutorials/examples/data/birth_life_2010.txt'
...
########## TO DO ############
#############################
w = tf.get_variable(shape = (), initializer=tf.random_normal_initializer(), name='W')
b = tf.get_variable(shape = (), initializer=tf.random_normal_initializer(), name='b')
...
def huber_loss(y, y_predicted):
  ...
  m = 1
  def f1(): return tf.square(y - y_predicted)/2
  def f2(): return tf.abs(y - y_predicted)
  cond = tf.abs(y - y_predicted)
  a = tf.cond(cond < m, f1, f2)
  return a
