...
class Word2Vec(object):
  def __init__(self, vocab_size, embed_size, num_sampled=NUM_SAMPLED):
    ...
    ########## TO DO ############
    #############################
    self.embed_matrix =  tfe.Variable(tf.random_uniform(shape=(vocab_size, embed_size)))
    self.nce_weight = tfe.Variable(tf.random_uniform(shape=(vocab_size, embed_size)))
    self.nce_bias = tfe.Variable(tf.random_uniform([vocab_size]))
  ...
  def compute_loss(self, center_words, target_words):
    ...
    ########## TO DO ############
    #############################
    loss = tf.reduce_mean(tf.nn.nce_loss(weights=self.nce_weight, biases=self.nce_bias, labels=target_words, 
                inputs=embed, num_classes=VOCAB_SIZE, num_sampled = self.num_sampled))
    return loss
...
def main():
  ...
  while num_train_steps < NUM_TRAIN_STEPS:
    print("Training step", num_train_steps)
    for center_words, target_words in tqdm(tfe.Iterator(dataset)):
      ...
      ########## TO DO ############
      #############################
      loss_batch, gradients = grad_fn(center_words, target_words)
